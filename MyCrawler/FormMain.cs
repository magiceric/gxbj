...
namespace MyCrawler
{
    public partial class FormMain : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ewbWeChat = new EWebBrowser();
            ewbWeChat.Parent = splitContainer1.Panel1;
            ewbWeChat.Dock = DockStyle.Fill;
            ewbWeChat.DocumentCompleted += new System.Windows.Forms.WebBrowserDocumentCompletedEventHandler(this.webBrowser1_DocumentCompleted);
        }
...
            //IPHostEntry localhost = Dns.GetHostByName(hostname);//方法已过期，只得到IPv4的地址  
            ...
            foreach (IPAddress addr in localhost.AddressList)
            {
                ...
                if (
                    sIP.IndexOf("192.168.1.") >= 0
                    || sIP.IndexOf("192.168.41.") >= 0
                    || sIP.IndexOf("59.108.") >= 0
                    )
                    return sIP;
            }
        ...
        private void InitialDBConnection()
        {
            ...
            if (sIP.IndexOf("192.168.1.118") >= 0
                || sIP.IndexOf("59.108.61.91") >= 0)
                strSource = "localhost";
            //外面
            if (strSource.Equals(""))
                strSource = "gxbj.vicp.net";
            ...
            try
            {
                ...
                toolStripStatusLabel1.Text = "Database(" + strSource + ") connected sucessfully.";
            }
            catch (Exception)
            {
                MessageBox.Show("警告：数据库连接失败。"+strSource);
                throw;
            }
        }
        ...
        private void buttonGo_Click(object sender, EventArgs e)
        {
            ...
            ewbWeChat.Navigate(sUrlHomepage, null, null, null);
        }
        ...
        private void webBrowser1_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            doc = ewbWeChat.Document;
            richTextBox1.Text = doc.Body.InnerHtml;

            //string strWebContent = GetWebContent(sUrlHomepage);

            textBox3.Text = doc.Url.LocalPath;

            //读取网页失败，尝试刷新
            if (
                doc.Url.LocalPath.IndexOf("dnserror") >= 0
                || doc.Url.LocalPath.IndexOf("/navcancl.htm") >= 0
                || doc.Url.LocalPath.Equals("blank")
                )
            {
                ewbWeChat.Navigate(sHref, null, null, null);
            }

            //登录处理
            //if (doc.Title == "微信公众平台")
            if (doc.All["loginBt"]!=null)
            {
                doc.All["account"].SetAttribute("value", "2547370594@qq.com");
                doc.All["pwd"].SetAttribute("value", "asd+123");
                doc.All["loginBt"].InvokeMember("click");
                textBox3.Text = doc.Url.LocalPath;
            }
        ...
        private void ProposerMatching()
        {
            try
            {
                ...
                MySqlCommand msqlUpdateTime;
                strSQL = "UPDATE updatetime SET lastupdatetime=NOW();";
                msqlUpdateTime = new MySqlCommand(strSQL, connUpdateTime);
                msqlUpdateTime.ExecuteNonQuery();

                strSQL = "SELECT lastupdatetime FROM updatetime;";
                msqlUpdateTime = new MySqlCommand(strSQL, connUpdateTime);
                DateTime dtLastUpdateTime = (DateTime)msqlUpdateTime.ExecuteScalar();

                Invoke((Action)delegate
                {
                    toolStripStatusLabel1.Text = "Proposers matching done.(" 
                        + dtLastUpdateTime.ToString()
                        + ")";
                });
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //if (!sUrlUserPage.Equals(""))
            //ewbWeChat.Navigate(sUrlUserPage, null, null, null);
            ewbWeChat.Navigate(sUrlHomepage, null, null, null);
        }
